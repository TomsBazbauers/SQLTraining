...
describe("Queries Across Tables", () => {
  ...
  it(
    ...
    async done => {
      const query = `SELECT ROUND(SUM(budget_adjusted), 2) AS total_budget, full_name AS director FROM movies 
  ...
  it(
    ...
    async done => {
      const query = `SELECT COUNT() AS count, keyword FROM movies
      JOIN movie_keywords ON movie_keywords.movie_id = movies.id
  ...
  it(
    ...
    async done => {
      const query = `SELECT COUNT() AS five_stars_count, genre FROM movies
  ...
  it(
    ...
    async done => {
      const query = `SELECT ROUND(avg(rating), 2) AS avg_rating, genre FROM movies
      JOIN movie_ratings ON movie_ratings.movie_id = movies.id
      JOIN movie_genres ON movie_genres.movie_id = movies.id
      JOIN genres ON movie_genres.genre_id = genres.id
      GROUP BY genre
