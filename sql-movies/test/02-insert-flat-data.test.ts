...
const insertMovies = (movies: Movie[]) => {
  return(
    `INSERT INTO movies (
      imdb_id, popularity, budget, budget_adjusted, revenue, revenue_adjusted, original_title, 
  ...
  movies.map(
    mov => `(
      ...
      '${mov.revenue}',
      '${mov.revenueAdjusted}',
      '${escape(mov.originalTitle)}',
      '${escape(mov.homepage)}',
      '${mov.tagline == undefined ? 0 : `${escape(mov.tagline)}`}',   
      '${escape(mov.overview)}',
      '${mov.runtime}',
      '${escape(mov.releaseDate)}'
      )`).join(",")
  );
};
