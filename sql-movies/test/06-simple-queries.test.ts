...
import { selectCount } from "../src/queries/select";
...
describe("Simple Queries", () => {
  ...
  it(
    ...
    async done => {
      const query = `SELECT ROUND(SUM(budget_adjusted), 2) AS total_budget, ROUND(SUM(revenue_adjusted), 1) AS total_revenue FROM movies`;
  ...
  it(
    ...
    async done => {
      const query = `SELECT COUNT(original_title) AS count, strftime("%Y", release_Date) AS year FROM movies 
  ...
  it(
    ...
    async done => {
      const query = `SELECT COUNT(rating) AS count, user_id AS user_id FROM movie_ratings 
      GROUP BY user_id 
  ...
  it(
    ...
    async done => {
      const query = `SELECT COUNT(rating) AS count, strftime("%m", time_created) AS month FROM movie_ratings 
