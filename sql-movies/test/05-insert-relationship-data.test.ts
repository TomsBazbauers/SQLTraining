...
const insertMovieGenres = (
  ...
  const filteredGenres = genreRows.filter(genre => genres.indexOf(genre.genre) != -1)
  return (`INSERT INTO movie_genres (movie_id, genre_id) VALUES` +
  filteredGenres.map(g => `(${movieId}, ${g.id})`).join(","));
};
...
const insertMovieActors = (
  ...
  const filteredActors = actorRows.filter(actor => actors.indexOf(actor.full_name) != -1)
...
const insertMovieDirectors = (
  ...
  const filteredDirectors = directorRows.filter(director => directors.indexOf(director.full_name) != -1)
...
const insertMovieKeywords = (
  ...
  const filteredKeywords = keywordRows.filter(keyword => keywords.indexOf(keyword.keyword) != -1)
...
const insertMovieProductionCompanies = (
  ...
  const filteredMovieProdComp = productionCompanyRows.filter(company => productionCompanies.indexOf(company.company_name) != -1)
  return(`INSERT INTO movie_production_companies(movie_id, company_id) VALUES` +
  filteredMovieProdComp.map(company => `(${movieId}, ${company.id})`).join(","));
};
